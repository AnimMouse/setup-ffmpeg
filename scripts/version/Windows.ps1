...
if ($env:version -eq 'release') {
  $latest_release = (Invoke-RestMethod https://endoflife.date/api/ffmpeg.json)[0].latest
  ...
  if ($status_code -eq '404') {
    $latest_release = (Invoke-RestMethod https://endoflife.date/api/ffmpeg.json)[1].latest
  }
